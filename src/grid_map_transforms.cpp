...
bool addFlux(grid_map::GridMap &grid_map,
             int radius_cells,
             const std::string &dist_trans_layer,
             const std::string &flux_layer) {
  if (!grid_map.exists(dist_trans_layer))
    return false;
  const grid_map::Matrix &data_distance_transform = grid_map[dist_trans_layer];
  grid_map.add(flux_layer, 0.0);
  grid_map::Matrix &data_flux = grid_map[flux_layer];
  const float valid_flux_expected = float(radius_cells * (radius_cells * 2 - 1) * 2);
  const float valid_flux_max = 2.0f * valid_flux_expected;
  const float valid_flux_min = 0.4f * valid_flux_max;

  for (grid_map::GridMapIterator iterator(grid_map); !iterator.isPastEnd(); ++iterator) {
    const grid_map::Index index_center = iterator.getUnwrappedIndex();
    if (!grid_map.isValid(index_center, dist_trans_layer)) continue;
    const float value_center = data_distance_transform(index_center(0), index_center(1));
    if(isnan(value_center)) continue;
    float flux = 0.f;
    bool valid_estimate = true;
    for (int horizontal_side_shift : {-radius_cells, radius_cells}) {
      for (int vertical_shift = - radius_cells + 1; vertical_shift < radius_cells; ++vertical_shift) {
        ...
        const float value = data_distance_transform(index_sample(0), index_sample(1));
        if (isnan(value) || !isfinite(value)) {
          valid_estimate = false;
          continue;
        }
          flux += value_center - value;
      }
    }
    if(!valid_estimate) continue;
    for (int vertical_side_shift : {-radius_cells, radius_cells}) {
    ...
    if(!valid_estimate) continue;
    if(flux < valid_flux_max && flux > valid_flux_min) {
      data_flux(index_center(0), index_center(1)) = 100.0;
    }
  }
  return true;
}
...
bool addSkeleton(grid_map::GridMap& grid_map,
                 ...
                 const std::string& skeleton_layer) {
  if (!grid_map.exists(input_layer))
    return false;

  grid_map::Matrix &grid_data = grid_map[input_layer];
...
//    for (size_t idx_y = 0; idx_y < size_y; ++idx_y) {
//      input[map_mat.cols * idx_x + idx_y] = (grid_data(idx_x, idx_y)) == 100.0 ? 255 : 0;
//      if(input[map_mat.cols * idx_x + idx_y] == 255) num_high++;
//      if(input[map_mat.cols * idx_x + idx_y] == 0) num_low++;
//    }
//  }
//  ROS_INFO("h %i %i",num_high, num_low);

  grid_map.add(skeleton_layer);
  grid_map::Matrix &data_normal(grid_map[skeleton_layer]);

  //Have to work around row vs column major representation in cv vs eigen
  Eigen::Matrix<u_int8_t , Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> data(data_normal.cast<u_int8_t>());

  cv::Mat skeleton_mat(data.rows(), data.cols(), CV_8U, data.data());
//  ROS_INFO("Map mat size %i %i", map_mat.rows, map_mat.cols);
//  for(int ix = 0; ix<skeleton_mat.rows; ix += 10) {
//    for(int iy = 0; iy<skeleton_mat.cols; iy += 10) {
//      if(skeleton_mat.at<u_int8_t>(ix, iy) > 0) {
//      ROS_INFO("skeleton_mat before value %i", map_mat.at<u_int8_t>(ix, iy));
//      }
//    }
//  }
  cv::Mat input_mat(size_x, size_y, CV_8U);
  cv::Mat output_mat(size_x, size_y, CV_8U);
  ...
  cv::ximgproc::thinning(input_mat, output_mat, 0);

  for (size_t idx_x = 0; idx_x < size_x; ++idx_x) {
    for (size_t idx_y = 0; idx_y < size_y; ++idx_y) {
      data_normal(idx_x, idx_y) =  output_mat.at<u_int8_t>(idx_x, idx_y);
    }
  }
